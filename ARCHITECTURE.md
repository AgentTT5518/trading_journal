...
> Last updated: 2026-05-13 (Added Journal Filter Toolbar; updated Component Map & Feature Log) | Updated by: Claude Code
...
| MoodHeatmap | `src/features/dashboard/components/mood-heatmap.tsx` | 2-month calendar grid color-coded by journal mood (1–5 scale); deduplicates multiple daily entries | getMoodHeatmapData |
| DashboardQueries | `src/features/dashboard/services/queries.ts` | getDashboardData, computeDashboardMetrics, computeMaxDrawdown, computeRMultipleStats, getMoodHeatmapData (all pure except async wrappers) | getTrades from trades feature, journalEntries table |
| JournalList | `src/features/journal/components/journal-list.tsx` | Card list of entries with category badge, mood emoji, trade count; inline toolbar for client-side filtering (category, mood, date range, search) | filterJournalEntries |
| JournalFilters | `src/features/journal/filters.ts` | Pure `filterJournalEntries(entries, filters)` + `isReverseDateRange` helpers — null-safe search, mood-null exclusion when specific mood selected, inclusive ISO-date bounds | — |
...
| Mood Heatmap | 2026-03-17 | Dashboard widget: 2-month calendar heatmap of daily journal mood (1–5). Color scale: 1=red-600 → 5=green-600. Multiple entries per day deduplicated by most-recent createdAt. No schema changes — reads existing journalEntries.mood column. | `src/features/dashboard/components/mood-heatmap.tsx`, `src/features/dashboard/services/queries.ts` (+getMoodHeatmapData), `src/features/dashboard/types.ts` (+MoodHeatmapDay), `src/app/(app)/dashboard/page.tsx`. 5 tests |
...
| Journal Filter Toolbar | 2026-05-13 | Client-side inline toolbar above journal entries — category select, mood select (1–5), date range (from/to ISO YYYY-MM-DD), free-text search across title + content. Pure `filterJournalEntries` helper extracted for unit tests. Reverse date range shows inline warning. Distinct empty states for "no entries" vs "no matches". Filter state is local (resets on navigation), matching TradeFilters pattern. | `src/features/journal/filters.ts` (new), `src/features/journal/components/journal-list.tsx`, `tests/features/journal/filters.test.ts` (new, 12 tests). 626 tests total |
